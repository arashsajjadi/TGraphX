...
from tgraphx.kg import (
    generate_synthetic_kg, RotatEModel,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RotatE KG benchmark (Experimental)")
    ...
    model = RotatEModel(n_e, n_r, embedding_dim=32, margin=6.0)
    ...
    cfg = KGTrainingConfig(num_epochs=epochs, batch_size=32 if args.small else 128,
                           loss_type="softplus", lr=0.001, seed=args.seed, device=args.device)
    ...
    report = {
        "task": "link_prediction", "model": "RotatE",
        ...
        "limitation_notes": ["Experimental; unit-rotation constraint enforced via normalisation."],
    }
    if args.output:
    ...
    if args.json:
        ...
        print(f"RotatE: loss={result['final_loss']:.4f} MRR={mrr} rt={runtime:.2f}s")
