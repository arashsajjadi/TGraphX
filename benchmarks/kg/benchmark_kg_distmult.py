...
from tgraphx.kg import (
    generate_synthetic_kg, DistMultModel,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="DistMult KG benchmark")
    ...
    model = DistMultModel(n_e, n_r, embedding_dim=32)
    ...
    report = {
        "task": "link_prediction", "model": "DistMult",
        ...
        "limitation_notes": ["Synthetic dataset only; DistMult symmetric relation limitation applies."],
    }
    if args.output:
    ...
    if args.json:
        ...
        print(f"DistMult: loss={result['final_loss']:.4f} MRR={mrr} rt={runtime:.2f}s")
